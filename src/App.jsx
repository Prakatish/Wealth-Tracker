...
var LOCAL_RULES = [
  {
    kw: ['SALARY', 'SALFEBRUARY', 'SALMARCH', 'BAINCAP', 'BAIN CAPABILITY'],
    cat: 'Income',
    sub: 'Salary',
  },
  { kw: ['RENT', 'HOUSE RENT'], cat: 'Household', sub: 'Rent' },
  {
    kw: ['ELECTRICITY', 'BESCOM', 'MSEDCL', 'TNEB'],
    cat: 'Household',
    sub: 'Electricity',
  },
  {
  ...
  {
    kw: ['HAIRCUT', 'SALON', 'SPA', 'SOIS BELLE'],
    cat: 'Personal Care',
    sub: 'Haircut',
  },
  {
  ...
  {
    kw: ['MUTUAL FUND', 'ZERODHA', 'GROWW'],
    cat: 'Savings',
    sub: 'Mutual Fund',
  },
  { kw: ['EMI', 'LOAN REPAY'], cat: 'Loan Repayment', sub: 'Personal Loan' },
  {
    kw: ['INSURANCE', 'MANIPALCIGNA', 'CIGNA'],
    cat: 'Health',
    sub: 'Insurance',
  },
  {
    kw: ['AMAZON', 'FLIPKART', 'MEESHO', 'MYNTRA', 'AJIO', 'MAX HYPERMARKET'],
    cat: 'Personal Care',
    sub: 'Clothes',
  },
...
// ─── pure localStorage store (no window.storage / API) ───────────────────────
var store = {
  get: function (key) {
...
function fmt(val, sym) {
  return (
    sym + Number(val).toLocaleString('en-IN', { maximumFractionDigits: 2 })
  );
}
...
function parseHDFCBank(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var desc =
      narration
        .replace(/UPI-|IMPS-|NEFT DR-|NEFT CR-|ACH C-|IB BILLPAY DR-|ATW-/g, '')
        .split('-')
        .pop()
        .trim() || narration.substring(0, 50);
...
function detectAndParseLocally(content) {
  if (content.indexOf('~|~') > -1) {
    ...
    if (cc.length > 0)
      return { txns: cc, label: 'Credit card: ' + cc.length + ' transactions' };
  }
  ...
  for (var i = 0; i < lines.length; i++) {
    if (
      /\d{2}\/\d{2}\/\d{2}/.test(lines[i]) &&
      lines[i].split(',').length >= 5
    ) {
      var bank = parseHDFCBank(content);
      if (bank.length > 0)
        return {
          txns: bank,
          label: 'Bank statement: ' + bank.length + ' transactions',
        };
      break;
    }
  }
...
function parseGenericCSV(content) {
  ...
  // Find column indices
  var dateCol = header.findIndex(function (c) {
    return c.indexOf('date') > -1;
  });
  var descCol = header.findIndex(function (c) {
    return (
      c.indexOf('narration') > -1 ||
      c.indexOf('description') > -1 ||
      c.indexOf('particular') > -1 ||
      c.indexOf('details') > -1 ||
      c.indexOf('remarks') > -1
    );
  });
  if (descCol === -1)
    descCol = header.findIndex(function (c) {
      return c.indexOf('ref') > -1 || c.indexOf('trans') > -1;
    });
  var debitCol = header.findIndex(function (c) {
    return (
      c.indexOf('debit') > -1 ||
      c.indexOf('withdrawal') > -1 ||
      c.indexOf('dr') === c.length - 2
    );
  });
  var creditCol = header.findIndex(function (c) {
    return (
      c.indexOf('credit') > -1 ||
      c.indexOf('deposit') > -1 ||
      c.indexOf('cr') === c.length - 2
    );
  });
  var amtCol = header.findIndex(function (c) {
    return c === 'amount' || c === 'amt';
  });
  ...
  for (var r = headerIdx + 1; r < lines.length; r++) {
    var row = lines[r].split(',').map(function (c) {
    ...
    if (dm1) {
      ...
      if (parseInt(dm1[1]) > 12) {
        parsedDate =
          yr + '-' + dm1[2].padStart(2, '0') + '-' + dm1[1].padStart(2, '0');
      } else {
        parsedDate =
          yr + '-' + dm1[1].padStart(2, '0') + '-' + dm1[2].padStart(2, '0');
      }
    } else if (dm2) {
      parsedDate =
        dm2[1] + '-' + dm2[2].padStart(2, '0') + '-' + dm2[3].padStart(2, '0');
    }
    ...
    var debit =
      debitCol > -1 ? parseFloat(row[debitCol].replace(/[,\s]/g, '')) || 0 : 0;
    var credit =
      creditCol > -1
        ? parseFloat(row[creditCol].replace(/[,\s]/g, '')) || 0
        : 0;
    var amt =
      amtCol > -1 ? parseFloat(row[amtCol].replace(/[,\s]/g, '')) || 0 : 0;
  ...
  return txns.length > 0
    ? {
        txns: txns,
        label: 'Parsed: ' + txns.length + ' transactions (review categories)',
      }
    : null;
}
...
function CustomPieTooltip(props) {
  ...
  return (
    ...
    >
      <p style={{ margin: '0 0 2px', fontWeight: 600, color: '#e2e8f0' }}>
        {d.name}
      </p>
      <p style={{ margin: 0, color: COLORS[d.index % COLORS.length] }}>
        {fmt(d.value, props.sym)}
      </p>
...
function TxnModal(props) {
  ...
  var catKeys = Object.keys(props.categories).filter(function (c) {
    return form.type === 'income' ? c === 'Income' : c !== 'Income';
  });
  return (
    <div
      style={{
    ...
    >
      ...
      >
        <h3 style={{ margin: '0 0 1rem', fontSize: 15, fontWeight: 700 }}>
          {props.txn ? 'Edit' : 'Add'} Transaction
        </h3>
          ...
          {form.category && (
            <div>
              ...
              <select
                ...
                onChange={function (e) {
                  setF('subcategory', e.target.value);
                }}
                style={is}
              >
                <option value="">Select subcategory</option>
                {(props.categories[form.category] || []).map(function (s) {
                  return (
          ...
          <button
            onClick={function () {
              if (form.description && form.amount)
                props.onSave(
                  Object.assign({}, form, { amount: parseFloat(form.amount) })
                );
            }}
            style={{
              flex: 1,
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 14px', fontSize: 12, color: '#64748b' }}>
          Select the account so expenses are attributed correctly.
        </p>
        <div
          style={{
            display: 'flex',
            ...
            return (
              <button
                ...
                style={{
                  padding: '10px 14px',
                  borderRadius: 10,
                  border: '2px solid ' + (sel ? c : '#2d2d3d'),
                  background: sel ? c + '22' : '#0f0f13',
                  color: '#e2e8f0',
                  cursor: 'pointer',
                  textAlign: 'left',
                  display: 'flex',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            onClick={function () {
              if (selAcct) props.onConfirm(selAcct);
            }}
            disabled={!selAcct}
            style={{
              flex: 1,
              ...
              borderRadius: 8,
              border: 'none',
              background: selAcct ? '#6366f1' : '#3730a3',
              color: '#fff',
              cursor: selAcct ? 'pointer' : 'not-allowed',
              fontWeight: 600,
            }}
          >
            Continue →
...
function CCDateModal(props) {
  ...
  var yearOptions = Array.from({ length: 5 }, function (_, i) {
    return now.getFullYear() - 1 + i;
  });
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 16px', fontSize: 12, color: '#64748b' }}>
          Credit card bills are usually paid the following month. How would you
          like to reflect these transactions?
        </p>
        <div
          style={{
            display: 'flex',
        ...
        >
          {[
            [
              ...
              'Keep the original dates from the statement',
            ],
            [
              'reflect',
              'Reflect in a different month',
              "Shift all entries to the month you'll pay the bill",
            ],
          ].map(function (opt) {
            return (
              <label
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border:
                    '2px solid ' + (option === opt[0] ? '#6366f1' : '#2d2d3d'),
                  background: option === opt[0] ? '#312e81' : 'transparent',
                }}
              >
                <input
                  type="radio"
                ...
                <div>
                  <div
                    style={{ fontSize: 13, fontWeight: 500, color: '#e2e8f0' }}
                  >
                    {opt[1]}
                  </div>
        ...
        {option === 'reflect' && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <select
              value={reflectMonth}
              onChange={function (e) {
                setReflectMonth(parseInt(e.target.value));
              }}
              style={{
                flex: 1,
                background: '#0f0f13',
            ...
            <select
              value={reflectYear}
              onChange={function (e) {
                setReflectYear(parseInt(e.target.value));
              }}
              style={{
                flex: 1,
                background: '#0f0f13',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            onClick={function () {
              props.onConfirm(
                option === 'reflect'
                  ? { month: reflectMonth, year: reflectYear }
                  : null
              );
            }}
            style={{
...
export default function App() {
  ...
  var yearOptions = Array.from({ length: 10 }, function (_, i) {
  ...
  useEffect(function () {
    try {
      ...
      var cat = store.get('wt:cat');
      if (cat) setCategories(JSON.parse(cat.value));
      var accts = store.get('wt:accts');
      if (accts) setAccounts(JSON.parse(accts.value));
  ...
  useEffect(
    function () {
      if (storageReady) store.set('wt:tx', JSON.stringify(transactions));
    },
    [transactions, storageReady]
  );
  useEffect(
    function () {
      if (storageReady) store.set('wt:cat', JSON.stringify(categories));
    },
    [categories, storageReady]
  );
  useEffect(
    function () {
      if (storageReady) store.set('wt:accts', JSON.stringify(accounts));
    },
  ...
  useEffect(
    ...
    [cycleStart, cycleEnd, currency, selMonth, selYear, storageReady]
  );
  ...
  function filterToCycle(txns, month, year) {
    ...
    var e =
      cycleEnd < cycleStart
        ? new Date(year, month + 1, cycleEnd)
        : new Date(year, month, cycleEnd);
  ...
  function isYearlyStatement(txns) {
    ...
    var mn = new Date(Math.min.apply(null, dates)),
      mx = new Date(Math.max.apply(null, dates));
    return (
      (mx.getFullYear() - mn.getFullYear()) * 12 +
        (mx.getMonth() - mn.getMonth()) >=
      3
    );
  }

  var getMonthTxns = useCallback(
    function () {
  ...
  function groupBy(arr, key) {
    ...
    return Object.keys(m).map(function (n) {
      return { name: n, value: m[n] };
    });
  }

  var expPieData = drillCat
    ? groupBy(
        monthTxns.filter(function (t) {
          return t.type === 'expense' && t.category === drillCat;
        }),
        'subcategory'
      )
    : groupBy(
  ...
  var savPieData = drillSavCat
    ? groupBy(
        monthTxns.filter(function (t) {
          return (
            t.category === 'Savings' &&
            (t.subcategory || 'Other') === drillSavCat
          );
        }),
        'description'
      )
  ...
  var savPieTotal = savPieData.reduce(function (a, b) {
    return a + b.value;
  }, 0);

  var acctExpTxns = monthTxns.filter(function (t) {
    return t.type === 'expense';
  });
  var acctPieMap = {};
  ...
  var acctPieData = Object.keys(acctPieMap).map(function (n) {
    return { name: n, value: acctPieMap[n] };
  });
  var acctPieTotal = acctExpTxns.reduce(function (a, b) {
    return a + b.amount;
  }, 0);
  ...
  function applyLocalResult(result) {
    ...
    var withIds = categorized.map(function (t, i) {
      return Object.assign({}, t, { id: 'p' + i });
    });
    ...
    if (isYearlyStatement(withIds)) {
      ...
      setCyclePickerMonth(latest.getMonth());
      setCyclePickerYear(latest.getFullYear());
  ...
  function processContent(content) {
    ...
    if (generic) {
      applyLocalResult(generic);
      return;
    }

    // 3. Nothing worked — show helpful error
    setParseError(
      "Could not auto-detect format. Please download your statement as CSV/Excel from your bank's net banking and try again. HDFC Bank & Credit Card statements are fully supported."
    );
    setParsing(false);
  }
  ...
  function applyMonthFilter(month, year) {
    ...
    if (filtered.length === 0) {
      setParseError('No transactions for ' + MONTHS[month] + ' ' + year + '.');
      setShowCyclePicker(true);
      return;
    }
    var withIds = filtered.map(function (t, i) {
      return Object.assign({}, t, {
        id: 'p' + i,
        category: t.category || '',
        subcategory: t.subcategory || '',
        autocat: t.autocat || 'low',
      });
    });
    setSelMonth(month);
    setSelYear(year);
    ...
    if (ext === 'pdf') {
      ...
      pdfLib
        ...
        .then(function (pdf) {
            ...
            return pdf
              ...
              .then(function (tc) {
                ...
                tc.items.forEach(function (it) {
                  ...
                  if (!rows[y]) rows[y] = [];
                  rows[y].push({ x: Math.round(it.transform[4]), str: it.str });
                });
                Object.keys(rows)
                  .map(Number)
                  .sort(function (a, b) {
          ...
          return doPage(1, '').then(function (text) {
            ...
            setParseDebug(
              'PDF: ' + pdf.numPages + 'p, ' + text.length + ' chars'
            );
            processContent(text);
          });
        })
        .catch(function (pe) {
          var msg = String(pe);
          if (
            (pe && pe.name === 'PasswordException') ||
            msg.indexOf('password') > -1
          ) {
            setParsing(false);
            setPendingFile(file);
            setParseError('password_required');
            return;
          }
          if (msg.indexOf('empty_pdf') > -1) {
            setParseError(
              'Image-based PDF. Please download the statement as CSV/Excel from net banking instead.'
            );
            setParsing(false);
            return;
          }
          setParseError('PDF parsing failed: ' + pe.message);
          setParsing(false);
        });
    } else if (ext === 'xls' || ext === 'xlsx') {
      var xlsLib = window.XLSX
        ? Promise.resolve()
        : loadScript(
      ...
      xlsLib
        ...
        .then(function (buf) {
          ...
          var csv = window.XLSX.utils.sheet_to_csv(
      ...
      file
        ...
        .catch(function (e) {
          setParseError('Read failed: ' + e.message);
          setParsing(false);
        });
    }
  }
  ...
  function confirmParsed() {
    var confirmed = parsedTxns.map(function (t) {
      return Object.assign({}, t, { id: 't' + Date.now() + Math.random() });
    });
    setTransactions(function (prev) {
      return prev.concat(confirmed);
    });
    setParsedTxns([]);
    setTab('Transactions');
  }
  ...
  function saveTxn(txn) {
    if (editTxn) {
      ...
      setTransactions(function (prev) {
        return prev.concat([Object.assign({}, txn, { id: 't' + Date.now() })]);
      });
  ...
  function exportData(format) {
    var rows = monthTxns.map(function (t) {
      return [
        t.date,
    ...
    if (format === 'csv') {
      ...
      var body = [
        'PERSONAL WEALTH TRACKER - ' + MONTHS[selMonth] + ' ' + selYear,
        sep,
        'Currency: ' + currency.code,
        'Cycle: Day ' + cycleStart + ' to ' + cycleEnd,
        '',
        'SUMMARY',
        'Income:   ' + fmt(income, sym),
        'Expenses: ' + fmt(expense, sym),
        'Savings:  ' + fmt(savings, sym),
        'Rate:     ' + savRate + '%',
        '',
        'TRANSACTIONS',
        new Array(81).join('-'),
  ...
  function exportJSON() {
    var data = {
      ...
      categories,
      accounts,
      settings: { cycleStart, cycleEnd, currency: currency.code },
    };
    downloadFile(
      JSON.stringify(data, null, 2),
      'wealthtrack_backup_' + new Date().toISOString().split('T')[0] + '.json',
      'application/json'
    );
  }
      ...
      try {
        ...
        if (data.settings) {
          ...
          if (data.settings.currency) {
            var c = CURRENCIES.find(function (x) {
              return x.code === data.settings.currency;
            });
            if (c) setCurrency(c);
          }
        }
  ...
  function prevMonth() {
    var m = selMonth === 0 ? 11 : selMonth - 1;
    var y = selMonth === 0 ? selYear - 1 : selYear;
    setSelMonth(m);
    setSelYear(y);
    setDrillCat(null);
  }
  function nextMonth() {
    var m = selMonth === 11 ? 0 : selMonth + 1;
    var y = selMonth === 11 ? selYear + 1 : selYear;
    setSelMonth(m);
    setSelYear(y);
    setDrillCat(null);
  }

  var uncatCount = parsedTxns.filter(function (t) {
    return !t.category || t.autocat === 'low';
  }).length;
  var autoCatCount = parsedTxns.filter(function (t) {
    return t.category && t.autocat === 'high';
  }).length;
  ...
  return (
    ...
    >
      <style>
        {
          '.gmob{display:none}.gdesk{display:flex}@media(max-width:640px){.g3{grid-template-columns:1fr!important}.g2{grid-template-columns:1fr!important}.gdesk{display:none!important}.gmob{display:block!important}.otx td,.otx th{padding:5px 4px!important;font-size:11px!important}}'
        }
      </style>
      ...
      >
        ...
        {mobileMenu && (
          <div style={{ borderTop: '1px solid #2d2d3d', padding: '8px 0' }}>
            {TABS.map(function (t) {
              return (
                <button
                  key={t}
                  ...
                  style={{
                    ...
                    width: '100%',
                    padding: '10px 1rem',
                    background: tab === t ? '#312e81' : 'none',
                    border: 'none',
                    color: tab === t ? '#c7d2fe' : '#94a3b8',
                    cursor: 'pointer',
                    fontSize: 14,
                    textAlign: 'left',
                  }}
          ...
          <div>
            ...
            >
              {[
                ...
                ['Net Savings', savings, savings >= 0 ? '#6366f1' : '#ef4444'],
              ].map(function (item) {
                return (
                  <div
                    key={item[0]}
            ...
            >
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        <Pie
                          ...
                          dataKey="value"
                          onClick={function (d) {
                            if (!drillCat) setDrillCat(d.name);
                          }}
                        >
                          {expPieData.map(function (_, i) {
                            return (
                              <Cell
                                key={i}
                                fill={COLORS[i % COLORS.length]}
                                cursor={drillCat ? 'default' : 'pointer'}
                              />
                            );
                          })}
                        </Pie>
                      ...
                      {expPieData.map(function (d, i) {
                        return (
                          ...
                          >
                            ...
                            {d.name}{' '}
                            {expPieTotal > 0
                              ? Math.round((d.value / expPieTotal) * 100)
                              : 0}
                            %
                          </span>
                        );
              ...
              >
                ...
                >
                  <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>
                    {drillSavCat ? drillSavCat : 'Savings'}
                  </h3>
                  {drillSavCat && (
                    <button
                      onClick={function () {
                ...
                {savPieData.length > 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        <Pie
                          ...
                          dataKey="value"
                          onClick={function (d) {
                            if (!drillSavCat) setDrillSavCat(d.name);
                          }}
                        >
                          {savPieData.map(function (_, i) {
                            return (
                              <Cell
                                key={i}
                                fill={COLORS[(i + 4) % COLORS.length]}
                                cursor={drillSavCat ? 'default' : 'pointer'}
                              />
                            );
                          })}
                        </Pie>
                      ...
                      {savPieData.map(function (d, i) {
                        return (
                          ...
                          >
                            ...
                            {d.name}{' '}
                            {savPieTotal > 0
                              ? Math.round((d.value / savPieTotal) * 100)
                              : 0}
                            %
                          </span>
                        );
                  ...
                  >
                    No savings tagged under "Savings" category
                  </p>
                )}
              </div>
            </div>
            ...
            >
              ...
              {acctPieData.length > 0 ? (
                <div>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value"
                      >
                        {acctPieData.map(function (d, i) {
                          var acct = accounts.find(function (a) {
                            return a.name === d.name;
                          });
                          var c = acct
                            ? ACCT_COLORS[acct.type] ||
                  ...
                  >
                    {acctPieData.map(function (d, i) {
                      ...
                      var c = acct
                        ? ACCT_COLORS[acct.type] || COLORS[i % COLORS.length]
                        : COLORS[i % COLORS.length];
                      return (
                        <span
                          key={d.name}
                        ...
                        >
                          ...
                          {d.name}{' '}
                          {acctPieTotal > 0
                            ? Math.round((d.value / acctPieTotal) * 100)
                            : 0}
                          %
                        </span>
                      );
                ...
                >
                  No account data — assign accounts to transactions
                </p>
              )}
            </div>
            ...
            >
              ...
              <p style={{ margin: '0 0 12px', fontSize: 11, color: '#475569' }}>
                How income flows through expenses into cash
              </p>
              {income > 0 || expense > 0 ? (
                <div>
                    ...
                    >
                      ...
                      <Bar
                        ...
                        shape={function (props) {
                          var d = wfd[props.index],
                            bg = props.background;
                          if (!bg) return <g />;
                          var bgH = bg.height || 200,
                            h = Math.abs(((d.end - d.start) / maxV) * bgH),
                            barY = bg.y + bgH - (d.end / maxV) * bgH;
                          return (
                            <g>
                              <rect
                                x={props.x}
                              ...
                              />
                              {props.index > 0 &&
                                props.index < wfd.length - 1 && (
                                  <line
                                    x1={props.x}
                                    x2={props.x + props.width + 8}
                                    y1={barY}
                                    y2={barY}
                                    stroke={d.fill}
                                    strokeWidth={1.5}
          ...
          <div>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {MONTHS[selMonth].slice(0, 3)} {selYear}
                  </span>
                  <button
                    onClick={nextMonth}
                    style={{
            ...
            >
              {monthTxns.length === 0 ? (
                ...
                >
                  No transactions for {MONTHS[selMonth]} {selYear}.
                </p>
              ) : (
                <table
                  className="otx"
          ...
          <div>
            ...
            >
              ...
              <p style={{ color: '#94a3b8', margin: '0 0 4px', fontSize: 14 }}>
                Upload your bank or card statement
              </p>
              <p style={{ color: '#10b981', margin: '0 0 4px', fontSize: 12 }}>
                ✓ HDFC Bank &amp; Credit Card parsed locally · ✓ Generic
                CSV/Excel auto-detection
              </p>
              <p style={{ color: '#475569', margin: '0 0 14px', fontSize: 12 }}>
                PDF, XLS, XLSX, CSV, TXT supported — no data sent to any server
              </p>
              <input
                ref={fileRef}
                type="file"
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: 'none',
                  background: parsing ? '#3730a3' : '#6366f1',
                  color: '#fff',
              ...
              {parseError === 'password_required' && (
                ...
                >
                  ...
                  <input
                    ...
                    onKeyDown={function (e) {
                      if (e.key === 'Enter' && pendingFile)
                        parseStatement(pendingFile, pdfPassword);
                    }}
                    style={{
                      width: '100%',
                      background: '#0f0f13',
                  ...
                  <button
                    onClick={function () {
                      if (pendingFile) parseStatement(pendingFile, pdfPassword);
                    }}
                    style={{
                      width: '100%',
                      padding: '8px',
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Yearly statement detected
                    </h3>
                    <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>
                      {allParsedTxns.length} transactions found. Select the
                      cycle to load.
                    </p>
                  </div>
                </div>
                {(function () {
                  var avail = {};
                  allParsedTxns.forEach(function (t) {
                    var d = new Date(t.date);
                    if (!isNaN(d.getTime())) {
                  ...
                  var sorted = Object.values(avail).sort(function (a, b) {
                  ...
                  return (
                    ...
                    >
                      {sorted.map(function (item) {
                        var sel =
                          cyclePickerMonth === item.month &&
                          cyclePickerYear === item.year;
                        return (
                          <button
                            key={item.year + '-' + item.month}
                            onClick={function () {
                              setCyclePickerMonth(item.month);
                              setCyclePickerYear(item.year);
                            }}
                            style={{
                              padding: '7px 12px',
                              borderRadius: 10,
                              border:
                                '1px solid ' + (sel ? '#6366f1' : '#2d2d3d'),
                              background: sel ? '#312e81' : '#0f0f13',
                              color: sel ? '#c7d2fe' : '#94a3b8',
                              cursor: 'pointer',
                              fontSize: 12,
                          ...
                          >
                            {MONTHS[item.month].slice(0, 3)} {item.year}
                            <span
                              style={{
                                display: 'block',
                                fontSize: 10,
                ...
                >
                  ...
                  <button
                    onClick={function () {
                      applyMonthFilter(cyclePickerMonth, cyclePickerYear);
                    }}
                    style={{
                      padding: '6px 18px',
                      borderRadius: 8,
              ...
              <div>
                ...
                >
                  <div>
                    ...
                    >
                      {parsedTxns.length} transactions parsed
                    </h3>
                    <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>
                ...
                >
                  {parsedTxns
                    .slice()
                    .sort(function (a, b) {
                      var aL = !a.category || a.autocat === 'low',
                        bL = !b.category || b.autocat === 'low';
                      return aL === bL ? 0 : aL ? -1 : 1;
                    })
                    .map(function (t) {
                      var needsAttn = !t.category || t.autocat === 'low';
                      var origIdx = parsedTxns.findIndex(function (p) {
                        return p.id === t.id;
                      });
                      function upd(k, v) {
                        setParsedTxns(function (prev) {
                        ...
                        >
                          ...
                          >
                            <span
                              style={{
                                ...
                                padding: '2px 6px',
                                borderRadius: 4,
                                background: needsAttn ? '#451a03' : '#052e16',
                                color: needsAttn ? '#facc15' : '#4ade80',
                              }}
                            >
                              {needsAttn ? '⚠ Manual' : '✓ Auto'}
                            </span>
                            {t.originalDate && (
                              <span style={{ fontSize: 10, color: '#f59e0b' }}>
                                reflected from {t.originalDate}
                              </span>
                            )}
                            <span
                              style={{
                          ...
                          >
                            ...
                            <input
                              ...
                              step="0.01"
                              onChange={function (e) {
                                upd('amount', parseFloat(e.target.value) || 0);
                              }}
                              style={{
                                background: '#0f0f13',
                                border: '1px solid #2d2d3d',
                          ...
                          >
                            <select
                              ...
                              style={{
                                flex: 1,
                                minWidth: 120,
                                background: needsAttn ? '#1c1008' : '#0a1a0f',
                                border:
                                  '1px solid ' +
                                  (needsAttn ? '#854d0e' : '#14532d'),
                                color: '#e2e8f0',
                            ...
                            {t.category && (
                              <select
                                value={t.subcategory || ''}
                                onChange={function (e) {
                                  upd('subcategory', e.target.value);
                                }}
                                style={{
                                  flex: 1,
                                  minWidth: 100,
                              ...
                              >
                                ...
                                {(categories[t.category] || []).map(function (
                                  s
                                ) {
                                  return (
                                    <option key={s} value={s}>
        ...
        {tab === 'Categories' && (
          <div>
                              ...
                              onClick={function () {
                                setCategories(function (prev) {
                                  return Object.assign({}, prev, {
                                    [cat]: prev[cat].filter(function (x) {
                                      return x !== s;
                                    }),
                                  });
                                });
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      <input
                        ...
                        onChange={function (e) {
                          ...
                          setNewSub(function (p) {
                            return Object.assign({}, p, { [cat]: v });
                          });
                        }}
                        onKeyDown={function (e) {
                          if (
                            ...
                            var v = newSub[cat].trim();
                            setCategories(function (p) {
                              return Object.assign({}, p, {
                                [cat]: p[cat].concat([v]),
                              });
                            });
                            setNewSub(function (p) {
                              return Object.assign({}, p, { [cat]: '' });
                            });
                          }
                        }}
                        style={{
            ...
            >
              ...
              <button
                onClick={function () {
                  if (newCat.trim() && !categories[newCat]) {
                    var v = newCat.trim();
                    setCategories(function (p) {
                      return Object.assign({}, p, { [v]: [] });
                    });
            ...
            <h2
              style={{ margin: '1.5rem 0 1rem', fontSize: 18, fontWeight: 700 }}
            >
              Manage Accounts
            </h2>
            <div
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 11, marginTop: 3, color: c }}>
                        {a.type}
                      </div>
                    </div>
                    <button
          ...
          <div style={{ maxWidth: 480 }}>
            ...
            >
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    value={cycleStart}
                    onChange={function (e) {
                      setCycleStart(Number(e.target.value));
                    }}
                    style={is}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={cycleEnd}
                    onChange={function (e) {
                      setCycleEnd(Number(e.target.value));
                    }}
                    style={is}
                  />
                </div>
              ...
              <div style={{ borderTop: '1px solid #2d2d3d', paddingTop: 16 }}>
                ...
                >
                  Saved in your browser's localStorage. Use backup to transfer
                  across devices.
                </p>
      ...
      {showAccountPicker && (
        <AccountPickerModal
          accounts={accounts}
          onAddAccount={function (a) {
            setAccounts(function (p) {
              return p.concat([a]);
            });
          }}
          onConfirm={onAccountConfirmed}
          onCancel={function () {
            setShowAccountPicker(false);
            setPendingParsedTxns([]);
          }}
